...
void table(vec& lines, vec& hexnum, un_mp& labels, mp& notLab, vec_pair& dupl) {
    ...
    std::map<std::string, std::string> instructions {
        {"add", "00"}, {"and", "01"}, {"not", "10"}, {"inc", "10"}, {"dec", "10"}, 
        {"mov", "10"}, {"bnz", "11"}
    };

    std::map<std::string, std::string> instructions2 { // Rm
    ...
    for (auto line: lines) {
        ...
        while (line_to_parse >> word) {
            ...
            if (it != instructions.end()) { 
                ...
                // get registers op code
                if (word == "not" || word == "mov") {
                    line_to_parse >> Rd;
                    line_to_parse >> Rn;
                    byte_str += registers.at(Rn);
                    byte_str += instructions2.at(word);
                    byte_str += registers.at(Rd);
                } 
                else if (word == "inc" || word == "dec") {
                    line_to_parse >> Rd;
                    // make Rn be 00
                    byte_str += "00" + instructions2.at(word);
                    byte_str += registers.at(Rd);
                }
...
void openFile(int argc, char** argv, vec& lines) {
    ...
    // std::ifstream inFile("fibosisc.s");
    std::string cur_line;
    bool emptyLine = true;
...
void createHexFile(const std::vector<std::string>& hexnum, char** argv) {
    ...
    // std::ofstream myFile("fibosisc.hex");
...
int main(int argc, char** argv) {
    ...
    if (argc == 4) {
        ...
        if (comment == "-l") {
            ...
            std::cerr << "Usage: siscas <asm file> <hex file> [-l]\n";
